...
<%@ page import="foodbank.domain.Category,foodbank.services.dao.mysql.FoodBankMySqlDAO,java.util.List,foodbank.domain.Donation,foodbank.domain.DonationItem,org.apache.commons.lang3.tuple.Pair" %>
...
<div class="back_container">
  <img src="../../resources/images/back_donor.png" alt="background" style="width:100%;">
  <div class="center-top">NURTURING THE DEPRIVED</div>
  <div class="center-center">Giving is not just about making a donation, it's about making a difference!<br/>- Kathy Calvin</div>
</div>
				...
				<div class="row">
					<div class="col-md-4 text-center animate-box">
						<div class="services">
							...
							<a onclick="selectTab('request-form');"><h3>Offer Donations</h3></a>
						</div>
					</div>
					<div class="col-md-4 text-center animate-box">
						<div class="services">
							<span><i class="icon-cloud-download"></i></span>
							<a onclick="selectTab('upload-csv');"><h3>Upload Donations</h3></a>
						</div>
					</div>
					<div class="col-md-4 text-center animate-box">
						<div class="services">
							<span><i class="icon-camera2"></i></span>
							<a onclick="selectTab('request-table');"><h3>Check Previous Donations</h3></a>
						</div>
					</div>
				</div>
				</form>
        ...
        <div id="request-form" style="visibility:hidden; width:80%; margin: 0 auto; padding: 5px; display: none;">
         <label for="requestForm" style="padding-left: 10%;"><b>Specify your Donation Contribution:</b></label>
         <form class="modal-content animate" action="donate" method="POST">
		   ...
		   <br><br>
		     ...
		     <label for="foodBank"><b>Donation Items:</b></label>
             <br/><br/>
        ...
        <div id="request-table" style="visibility: hidden; width: 100%; margin: 0 auto; padding: 5px; display: none;">
         <label for="requestTable" style="padding-left: 10%;"><b>Previous Donations:</b></label>
		<%
		System.out.println("Building page for user " + request.getSession().getAttribute("userId"));
			...
			List<Pair<Donation, List<DonationItem>>> donationsData = FoodBankMySqlDAO.getInstance().getPersistedDonations((Integer) request.getSession().getAttribute("userId"));
			System.out.println("Logged - " + donationsData);
			request.setAttribute("donationsData", donationsData);
		%>
         <form class="modal-content animate">
            <table class="table table-striped table-hover">
              <thead style="background-color:NavajoWhite;">
                <tr>
                  ...
                  <th>Comments</th>
                  <th>Donated On</th>
                  <th>View Donated Items</th>
                </tr>
              </thead>
              <tbody>
	          <c:forEach items="${donationsData}" var="donationData">
                 <tr>
                   <td>${donationData.left.donationID}</td>
                   <td>${donationData.left.preferredBeneficiary}</td>
                   <td>${donationData.left.appointmentTimeString}</td>
                   <td>${donationData.left.frequency}</td>
                   <td>${donationData.left.comments}</td>
                   <td>${donationData.left.updTimeString}</td>
                   <td><a onclick="displayDonationData(${donationData.left.donationID})">View</a></td>
                 </tr>
              </c:forEach>
              </tbody>
            </table>
			...
			<span id="selectedDonationItem" style="display: none; visibility: hidden;"></span>
            <label id="donationItemsTableLabel" style="padding-left: 10%;"></label>
            <c:forEach items="${donationsData}" var="donationData">
            <form id="donationItems${donationData.left.donationID}" style="display: none; visibility: hidden;" class="modal-content animate">
            <table class="table table-striped table-hover">
              <thead style="background-color:NavajoWhite;">
                <tr>
                  <th>Category</th>
              ...
              <tbody>
	            <c:forEach items="${donationData.right}" var="donationDataItem">
                 <tr>
                   <td>${donationDataItem.categoryID}</td>
                   <td>${donationDataItem.foodItem}</td>
                   <td>${donationDataItem.quantity}</td>
                   <td>${donationDataItem.units}</td>
                   <td>${donationDataItem.expirationDate}</td>
	             </tr>
	            </c:forEach>
              </tbody>
            </table>
		...
		<div id="upload-csv" class="container" style="visibility: hidden; width: 100%; margin: 0 auto; padding: 5px; display: none;">
			<form class="modal-content animate" action="upload" method="POST" enctype="multipart/form-data">
				...
				<input type="text" name="userType" value="donor" style="visibility: hidden; display: none;" />
    ...
    <script>
		...
		function displayDonationData(donationId) {
			document.getElementById("donationItemsTableLabel").innerHTML = "<b>Donation Items for Id " + donationId + "</b>";
			document.getElementById("donationItems" + donationId).style="visibility: visible; display: block;";
			if (document.getElementById("selectedDonationItem").innerHTML != "") {
				document.getElementById(document.getElementById("selectedDonationItem").innerHTML).style="visibility: hidden; display: none;";
			}
			document.getElementById("selectedDonationItem").innerHTML = "donationItems" + donationId;
		}
